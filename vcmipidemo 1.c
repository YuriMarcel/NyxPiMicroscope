...
int  process_capture(unsigned int pixelformat, char *st, int dx, int dy, int pitch, int netSrvOutIff1, VCImgNetCfg *imgnetCfg, int fileOutIff1, int frameNr, char *pcFramebufferDev, VCWhiteBalCfg *cfgWB)
{
	...
	{	// Get the current time
		...
		snprintf(acFilename, sizeof(acFilename), "/home/RayPairNyx/images_saved/img%d_%05d_%d", mLED.LED_number, mLED.timestamp, frameNr);//comment out when the LED number is not needed
...
int  change_options_by_commandline(int argc, char *argv[], int *shutter, float *gain, int *maxCaptures, int *fileOutIff1, int *bufCount, int *fps)
{
	...
	*fileOutIff1= 1; //Activating file output of captures
	...
	while ((opt = getopt(argc, argv, "s:i:bpL")) != -1) 
	{
		switch (opt) 
		{
			case 's':
			...
			case 'p':
				*fps = atol(optarg);
				...
				break;
			case 'L':
				mLED.LED_number = atoi(optarg); // Using atoi to handle leading zeros properly
				printf("LED number %d.\n", mLED.LED_number);
			...
			default:
				...
				printf("  -L Set the LED number.\n");
				printf("Example usage:  ./yuriscope_cam -s 5000 -i 5 -b 3 -p 30 -L 112\n");
